<template>
  <div class="home">
    ...
    <div class="container search">
      <input @keyup.enter="$fetch" type="text" placeholder="Search .." v-model="searchInput">
      <button @click="clearSearch" v-show="searchInput !== ''" class="button">Clear Search</button>
    </div>

    <loading-comp v-if="$fetchState.pending" />
  ...
  methods: {
    ...
    async getMovies() {
     const data = axios.get(
        `https://api.themoviedb.org/3/movie/now_playing?api_key=258273c9537200afcfd287f15a10f6d6&language=en-US&page=1`
      )
    ...
    async searchMovies() {
      const data = axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=258273c9537200afcfd287f15a10f6d6&language=en-US&page=1&query=${this.searchInput}`
      )
      const result = await data 
      result.data.results.forEach((movie) => {
